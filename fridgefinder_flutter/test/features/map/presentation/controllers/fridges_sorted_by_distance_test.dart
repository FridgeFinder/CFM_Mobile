...
void main() {
  ...
  group('fridgesSortedByDistanceProvider Tests', () {
    ...
    test('returns fridges without distance when location is not available', () async {
      // Mock location access disabled
      final locationAccessNotifier = container.read(locationAccessProvider.notifier);
      ...
      final fridgesWithDistance = container.read(fridgesSortedByDistanceProvider);
    ...
    test('provider uses select() to avoid unnecessary recalculations',
        () async {
      await container.read(fridgeListProvider.future);
    ...
    test('FridgeWithDistance model holds fridge and distance correctly',
        () async {
      final testFridge =
          (await container.read(fridgeListProvider.future)).first;
    ...
    test('uses granular location watching via select()', () async {
      ...
      final fridgesWithDistance1 = container.read(fridgesSortedByDistanceProvider);
      expect(fridgesWithDistance1, isNotEmpty);

      // Access again - should be cached
      final fridgesWithDistance2 = container.read(fridgesSortedByDistanceProvider);
      expect(identical(fridgesWithDistance1, fridgesWithDistance2), isTrue,
          reason:
